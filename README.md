# اختبار Opus 5 — سبعة برومبتات في ليلة واحدة
...
ليلة **٢٤ → ٢٥ يوليو ٢٠٢٦** أطلقت سبع مهام صعبة على Opus 5، واحدة تلو الأخرى، وصوّرت النتيجة.

هذا المستودع فيه **البرومبت الحرفي** لكل مهمة — نفس النص الذي كتبته، بلا تنقيح — و**التكلفة الفعلية** لكل بناء.

ستة منها بصرية: كل واحد **ملف HTML واحد مكتفٍ بذاته**، `three.js` من CDN، بلا ملفات أصول مرفقة، يُشغَّل بفتح الملف. هذا القيد هو الاختبار نفسه — يزيل نظام البناء ولا يترك شيئًا بين البرومبت والنتيجة.

والسابع مختلف تمامًا: **عمل معرفي** — لا رسوميات، فقط بحث وحُكم وتحليل مالي.
...
## الرقم الذي لا يعرضه أحد

| | التوكنات | النسبة | التكلفة |
|---|---:|---:|---:|
| قراءات الكاش | 377.3M | 97.5% | $188.65 |
| المخرجات | 3.66M | 0.9% | $91.38 |
| كتابات الكاش | 6.09M | 1.6% | $38.06 |

المخرجات النهائية للبناءات البصرية ≈ **840 كيلوبايت** من HTML.
الوصول إليها استهلك **٣٨٧ مليون توكن** — أي ما يقارب **٤٦٠ ضعفًا** من التوكنات المستهلكة مقابل المُسلَّمة.

هذه هي الصورة الحقيقية للبرمجة الوكيلية (agentic coding)، ونادرًا ما يعرضها أحد.

### اقرأ هذا قبل أن تقتبس أي رقم

- **هذه تكلفة مكافئة لأسعار الـ API، وليست بالضرورة ما دُفع فعلًا.** إذا شُغّلت الجلسة على اشتراك بدل الفوترة بالاستهلاك، فالمدفوع هو سعر الاشتراك.
- التسعير على أسعار Opus 5: `$5/M` إدخال · `$25/M` إخراج · كتابة كاش `$6.25/M` · قراءة كاش `$0.50/M`.
- **تسعير كتابة الكاش يفترض مهلة الـ ٥ دقائق (×1.25).** لو استُخدمت مهلة الساعة (×2) لارتفع ذلك البند ~60%، والسجلات لا توثّق أيهما استُخدم.
- **الأرقام هي مجاميع جلسات كاملة**، فأي نقاش جانبي في نفس النافذة محسوب ضمنها.
- <a name="fn1"></a>**\*** تكلفة Apple موزّعة على جلستين متداخلتين: **$19.29** لجزء البناء نفسه، و**$12.35** للجلسة التالية التي دمجت الملف النهائي. دقّتها أقل من الستة الأخرى.
- الإدخال الطازج غير المخزّن في الكاش عبر كل البناءات: **~3,100 توكن**. لا شيء عمليًا.

---

## ملاحظة قبل أن تنسخ أي برومبت

البرومبتات هنا **كما كُتبت وشُغّلت** — بلا تجميل.

في النسخ الأصلية أضفت لاحقًا كتلة `STOP. READ THIS FIRST` لأصوّر النموذج وهو *يفكّر* دون أن يعيد البناء من الصفر أمام الكاميرا. **تلك الكتلة محذوفة من هنا** لأن ما تراه في الأسفل هو النص الصالح للتشغيل الحقيقي.

سطور مثل `put it in b projects` أو `/effort max` مذكورة كما هي لأنها كانت جزءًا من الرسالة فعلًا.
